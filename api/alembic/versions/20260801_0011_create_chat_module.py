...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint("kind IN ('GERAL', 'PROPOSTA')", name=op.f("ck_chat_conversations_kind")),
        sa.CheckConstraint("status IN ('ATIVA', 'FINALIZADA')", name=op.f("ck_chat_conversations_status")),
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("now()"), nullable=False),
        sa.CheckConstraint("message_type IN ('MENSAGEM', 'PERGUNTA')", name=op.f("ck_chat_messages_message_type")),
        sa.CheckConstraint("question_status IS NULL OR question_status IN ('AGUARDANDO_RESPOSTA', 'RESPONDIDA')", name=op.f("ck_chat_messages_question_status")),
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("now()"), nullable=False),
        sa.CheckConstraint("notification_type IN ('MENSAGEM', 'OBSERVACAO', 'MENCAO')", name=op.f("ck_chat_notifications_notification_type")),
