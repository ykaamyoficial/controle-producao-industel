...
class FlowReviewDialog(QDialog):
    ...
    def _build_setup_page(self) -> QWidget:
        ...
        layout.addWidget(QLabel("Fluxo padrao"))
        ...
        layout.addWidget(self.default_route_combo)

        scope_frame = QFrame()
        scope_frame.setObjectName("Panel")
        scope_layout = QVBoxLayout(scope_frame)
        scope_layout.addWidget(QLabel("Alcance"))
        ...
        self.preserve_checkbox = QCheckBox("Preservar excecoes existentes")
        self.preserve_checkbox.setChecked(True)
        self.overwrite_checkbox = QCheckBox("Sobrescrever excecoes ja cadastradas")
        ...
        continue_btn = ModernButton("Continuar", "next", accent=True)
        continue_btn.clicked.connect(self._continue_to_review)
    ...
    def _build_review_page(self) -> QWidget:
        ...
        self.review_tree.header().setSectionResizeMode(COL_DESCRICAO, QHeaderView.Stretch)
        ...
        self.validation_button.setObjectName("GhostButton")
    ...
    def _populate_review_tree(self) -> None:
        ...
        for column in range(len(_ITEM_COLUMNS)):
            if column != COL_DESCRICAO:
                self.review_tree.resizeColumnToContents(column)
    ...
    def _build_item_row(self, proposal: FlowReviewProposal, item: FlowReviewItem, tree_item: QTreeWidgetItem) -> None:
        ...
        tree_item.setText(COL_ROTA_ANTERIOR, route_label(item.original_route))
        tree_item.setData(0, Qt.UserRole, item)

        key = (item.proposal_id, item.item_id)
        route_combo = QComboBox()
        for route in DEFAULT_ROUTE_CHOICES:
            route_combo.addItem(route_label(route), route)
        ...
        if not item.is_editable:
            route_combo.setToolTip(item.lock_reason or "Item bloqueado para alteracao de fluxo.")
    ...
    def _refresh_summary(self) -> None:
        ...
        if issues:
            ...
            self.validation_button.setText(f"Nao e possivel salvar: {preview}{suffix}")
    ...
    def _focus_first_problem(self) -> None:
        ...
        if not issues:
            return
        target_item_id = issues[0].item_id
    ...
    def _confirm_and_save(self) -> None:
        ...
        if answer != QMessageBox.Yes:
            return
        self._save(proposals_to_save)

    def _save(self, proposals_to_save: list[FlowReviewProposal]) -> None:
        self._saving = True
        ...
        self._save_thread = start_worker(
            self,
            lambda: self._save_operation(proposals_to_save),
            self._save_success,
            self._save_error,
        )

    def _save_operation(self, proposals_to_save: list[FlowReviewProposal]) -> list[dict]:
        results = []
    ...
    def _save_success(self, results: list[dict]) -> None:
        self._saving = False
        self.stack.setEnabled(True)
        ...
        if not conflicts and not errors:
            ToastNotification(self.window(), f"{len(saved)} proposta(s) salva(s) com sucesso.", "success")
            self.accept()
            return
