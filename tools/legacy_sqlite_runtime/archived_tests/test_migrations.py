...
from tools.legacy_sqlite_runtime import production_repository
from tools.legacy_sqlite_runtime.migration_runner import MigrationError, apply_migrations, migration_status
...
class MigrationSafetyTests(unittest.TestCase):
    ...
    def test_copy_of_current_database_migrates_without_data_loss(self):
        if not REAL_DB.exists():
            self.skipTest("Banco SQLite real nao existe no ambiente PostgreSQL-only.")
    ...
    def test_backup_file_exists_and_is_valid(self):
        ...
        if not backups:
            self.skipTest("Backups SQLite legados nao existem no ambiente PostgreSQL-only.")
